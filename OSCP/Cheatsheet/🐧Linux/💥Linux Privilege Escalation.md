...
※SGIDの場合はバイナリ所有グループに属するユーザーに対しLatMovが可能
...
![[Pasted image 20230627095059.png]]
...
![[Pasted image 20230626181707.png]]
...
- ⚠️カーネルエクスプロイト手法の列挙はSearchsploitだと非効率なことが多いため、LinPEAS.shを使用する
	- LinPEASの結果の"Executing Linux Exploit Suggester"を確認
...
- 🚨カーネルバージョンとディストロが一致していて公開エクスプロイトが利用可能に見えても、バックポート（※）で対象CVEへのパッチ当てが完了していることがあるため、公式サイトで確認して使うこと
	- （※）より新しいバージョン用の部品を古いバージョンに適用すること

💡以下手順のショートカット：以下のように検索する
```zsh
...
![[Pasted image 20250909131226.png]]
$$USNの修正済みバージョン情報$$


![USNの修正済みバージョン情報](Pasted%20image%2020250909131226.png)
...
- LinPEASでは以下のように表示される
	- 💡`/usr/share/keyrings`や`ubuntu-keyring...`は公開鍵リングなので無視
```zsh
