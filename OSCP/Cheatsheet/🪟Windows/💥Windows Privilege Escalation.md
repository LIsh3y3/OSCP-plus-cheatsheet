...
3. [権限関連の知識、コマンド](../Common/権限関連の知識、コマンド.md#🪟Windows)に記載の通り、icaclsで書込み可能であっても実際には書込みできないケースもあるので、害のないファイルを作成して実際に書き込めるかどうかを検証する
```powershell
echo 'test' > '<writable_dir>\test.txt'
type '<writable_dir>\test.txt'
```

4. [💥Windows Privilege Escalation](#バイナリのペイロード)を用意し、本来意図されていた実行ファイルよりも前に解釈されるファイル名に名前を変更し、書込み可能なディレクトリに配置する
```powershell
...
- [💥Windows Privilege Escalation](#PowerUpの準備)
...
3. ステップ１の"AbuseFunction"の実行
	- [💥Windows Privilege Escalation](#PowerUpの出力)
```powershell
...
1. [🔍Windows Local Enumeration](🔍Windows%20Local%20Enumeration.md#サービスの列挙)で、"SERVICE_START_NAME" : LocalSystemであるサービスを列挙する
...
1. [🔍Windows Local Enumeration](🔍Windows%20Local%20Enumeration.md#Scheduled%20Taskの列挙)で、PrivEscに繋げられるタスクを列挙する
...
2. 攻撃者のマシン上で[💥Windows Privilege Escalation](#バイナリのペイロード)によってexeファイルのペイロードを用意し、ターゲットマシンに転送する
...
3. 特定のWindows特権の悪用
	- [💥Windows Privilege Escalation](#Windows特権のエクスプロイト)
...
2. 攻撃者マシンで[💥Windows Privilege Escalation](#バイナリのペイロード)を用意し、ターゲットマシンに転送する
3. ⇩ペイロードを実行する：
...
>- [用語](../../Misc/用語.md#SAM(Security%20Account%20Manager))、[用語](../../Misc/用語.md#SYSTEM)
...
>- GUIアクセスがないときは、下に説明するステップ１、２の方法で任意のサービス・バイナリの所有権を取得し、[💥Windows Privilege Escalation](#Service%20Exploits%20-%20Service%20Binary%20Hijacking)を実施する
...
```
- 参考：[Module 14：Fixing Exploits](../../PEN-200/Module%2014：Fixing%20Exploits.md#クロスコンパイル環境の準備・実行)
