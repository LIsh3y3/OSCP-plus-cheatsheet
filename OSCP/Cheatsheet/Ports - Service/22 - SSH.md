...
![[Pasted image 20251227113634.png]]
...
![[Pasted image 20250330100126.png | 350]]
$$公開鍵認証フロー$$
...
- SSHアクセス
```zsh
ssh -i <private_key> -p <port> <username>@<TargetIP|Domain>
```
- ⚠️：sshd.configで、`PasswordAuthentication no`、`PubkeyAuthentication yes`となっていないと、パスワードの入力を求められる
...
...
```
- →対策：`known_hosts`を削除する。([22 - SSH](22%20-%20SSH.md#known_hostsとは))
```zsh
rm /.ssh/known/hosts
```
...
![[Pasted image 20251202065433.png]]
...
- 初期侵入できていない段階で、パストラバーサルの脆弱性を用いてクライアントから秘密鍵を抽出し、その秘密鍵でSSH接続する：
	- [⚡️Path traversal](../../../BSCP/Server-side/Path%20traversal/⚡️Path%20traversal.md#SSH%20private%20keyの抽出)
	- [22 - SSH](22%20-%20SSH.md#SSH%20Private%20Keyの種類)
- パスフレーズがある場合：ssh2john([🐈‍⬛Password Crack - JtR・Hashcat](../../Tools/🐈‍⬛Password%20Crack%20-%20JtR・Hashcat.md#ハッシュ値の整形))
...
```zsh
hydra -C /usr/share/seclists/Passwords/Default-Credentials/ssh-betterdefaultpasslist.txt <TargetIP> ssh -t 1
```
...
- [Tunneling Through Deep Packet Inspection](../Stealth&Evasion/Port%20Redirection/Tunneling%20Through%20Deep%20Packet%20Inspection.md#HTTP%20Tunneling%20w/%20Chisel)等の方法(ポートフォワーディング)で確立した*既存の*SOCKSプロキシ経由でSSH接続したいとき
	- [Port Redirection & SSH Port Forwarding](../Stealth&Evasion/Port%20Redirection/Port%20Redirection%20&%20SSH%20Port%20Forwarding.md#SSH%20Local%20Dynamic%20Port%20Forwarding)の`-D`のように、SSHがSOCKSプロキシを*新規で*用意して接続はできるが、既存のSOCKSプロキシ経由でのアクセスは不可
...
```zsh
ssh -oHostKeyAlgorithms=+ssh-rsa -oPubkeyAcceptedAlgorithms=+ssh-rsa <username>@<TargetIP>
```