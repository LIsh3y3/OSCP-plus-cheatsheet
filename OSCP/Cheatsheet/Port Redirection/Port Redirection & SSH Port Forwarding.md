- 関連ノート：
	- [HTTP・DNSトンネリング（Chisel, Ligolo-ng, Dnscat2）](HTTP・DNSトンネリング（Chisel,%20Ligolo-ng,%20Dnscat2）.md)

>[!TIP]
>OSCP試験においては、[Ligolo-ng](HTTP・DNSトンネリング（Chisel,%20Ligolo-ng,%20Dnscat2）.md#Ligolo-ng)もしくは[HTTP Tunneling w/ Chisel](HTTP・DNSトンネリング（Chisel,%20Ligolo-ng,%20Dnscat2）.md#HTTP%20Tunneling%20w/%20Chisel)で十分だった。
...
![](../../Images/Pasted%20image%2020260315093727.png)
...
![](../../Images/Pasted%20image%2020260315093651.png)
...
![](../../Images/Pasted%20image%2020250920195209.png)
...
- ❌近年のFWに実装されているDeep Packet Inspection(DPI)機能により、悪意ある通信は検知され遮断されてしまう
	- →HTTP通信が許可されている場合は[HTTP・DNSトンネリング（Chisel, Ligolo-ng, Dnscat2）](HTTP・DNSトンネリング（Chisel,%20Ligolo-ng,%20Dnscat2）.md)を使えば、DPIは回避可能
...
- SSHローカルポートフォワーディング
	- ローカルマシン(SSHクライアント Jump Host01 )のポートを、SSH サーバー（Jump Host02）経由で別のサーバーのポート（Target）に転送する。

- SSHローカルダイナミックポートフォワーディング
	- ローカルマシン(SSHクライアント)のポートを、SSH サーバー経由で別のサーバーの任意ポートに転送する。

- SSHリモートポートフォワーディング
	- SSH サーバー側のポートを、ローカルマシン(SSHクライアント)経由で別のサーバーのポートに転送する。
	- Jump Hostを介して社内NWにアクセスすることや、FWをバイパスして特定のポートにアクセスすることに使われる

- ==SSHリモートダイナミックポートフォワーディング==
	- SSHサーバー側のポートを、ローカルマシン(SSHクライアント)経由で別のサーバーの任意ポートに転送する。
	- SSH Port Forwardingの中では**最も成功率が高い**ので、これを優先的に使う。

- イメージ図は🔗[SSHを使用したポート転送やトンネリングの設定をわかりやすく図示したビジュアルガイド - Gigazine](https://gigazine.net/news/20240924-ssh-tunnel-port-forwarding/)を参照
...
> - →[SSH Remote Port Forwarding](Port%20Redirection%20&%20SSH%20Port%20Forwarding.md#SSH%20Remote%20Port%20Forwarding)のほうが成功確率高い
...
- [SSH Local Port Forwarding](Port%20Redirection%20&%20SSH%20Port%20Forwarding.md#SSH%20Local%20Port%20Forwarding)と同じく、現実では失敗する可能性が高い
	- →[SSH Remote Dynamic Port Forwarding](Port%20Redirection%20&%20SSH%20Port%20Forwarding.md#SSH%20Remote%20Dynamic%20Port%20Forwarding)
