...
| コマンド名               |                                                                                                                          用途 | 実行条件                                   |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------: | -------------------------------------- |
| `sekurlsa::pth`     |                                                                        ・PtH PtK 攻撃用のコマンド<br>・指定したユーザー名でハッシュを使って別プロセスを偽装起動する | SeDebugPrivilege 権限もしくはローカルの SYSTEM 権限 |
...
| `kerberos::golden` | 任意ユーザ／グループ向けに偽造の TGT または Service Ticket を作成（Golden Ticket / Silver Ticket の発行） | ・[[#Golden Ticket]]<br>・[[#Silver Ticket]]                                           |
...
```
- 🚨「現在の自分のセッション」にチケットを注入するため、注入したチケットのLUIDと自分のLUIDが一致していないと、コマンド実行時にそのチケットが使われない（[[ADの基本#チケットの形式]]）
...
- 特定のサービスに限定してDomain Adminレベルのアクセスが可能になるチケット(TGS)のこと
	- TGSがサービスアカウントのパスワードハッシュで暗号化されてさえいれば、チケットの中身のユーザーは盲目的に信用してしまう
...
- SPN
	- [[🔍AD Enumeration#SPNの列挙]]
- ターゲットのFQDN
	- [[🔍AD Enumeration#SPNの列挙]]
![[Pasted image 20251013140928.png]]
$$spn列挙と使用する値$$
...
- ローカルサービスの場合は[[Tunneling Through Deep Packet Inspection]]と組み合わせて実行する
...
- [[#DCSync攻撃]]と同じ権限を用いて、データの複製ではなく改ざんをする
